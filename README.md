...
- **外部跳转**：通过 `webhub://` scheme 被其他应用或网页唤起，按 URL 前缀匹配已存在的子应用并直达其内页
...
WebHub 对外暴露 `webhub://` scheme，目前支持可被其他应用或网页唤起，按 URL 前缀匹配已存在的子应用并直达其内页。

```
webhub://jump/auto?url=<URL 编码的目标网址>
```

例如子应用配置网址为 `https://example.com/`，调用 `webhub://jump/auto?url=https%3A%2F%2Fexample.com%2Fsubpage` 即可打开该子应用并直达 `https://example.com/subpage`。
